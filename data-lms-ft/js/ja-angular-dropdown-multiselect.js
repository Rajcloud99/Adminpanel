! function(angular) {
	...
	}]), directiveModule.directive("ngDropdownMultiselect", ["$filter", "$document", "$compile", "$parse", "$timeout", function($filter, $document, $compile, $parse, $timeout) {
		return {
			restrict: "AE",
			scope: {
				selectedModel: "=",
			...
			link: function($scope, $element, $attrs) {
				// debugger
				$scope.$watch('input.searchFilter',function(newVal){
					$timeout(function(){
						$scope.jaSearchInput = newVal;
						$scope.$apply();
						$scope.jaSearchTrigger();
					});
				});
