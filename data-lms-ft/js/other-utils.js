...
materialAdmin.factory('DatePicker', function () {
	...
	datePickerObj.minDate=new Date().getMonth()<3 ? new Date(new Date().getFullYear()-1, 3, 1) : new Date(new Date().getFullYear(), 3, 1);
	datePickerObj.maxDate=new Date().getMonth()<3 ? new Date(new Date().getFullYear(), 2, 31) : new Date(new Date().getFullYear()+1, 2, 31);
...
materialAdmin.filter('arrayOfGrToString2', function () {
	return function (input, ele) {
		let returnObj;
		if (Array.isArray(input) && ele)
			returnObj = (input.map(obj => obj[ele]['grNumber']) || []).join(', ');
		else
			returnObj = typeof returnFalseString !== 'undefined' ? returnFalseString : 'NA';
...
materialAdmin.filter('arrayOfIntermitPoint', function () {
	return function (input) {
		let returnObj;
		if (Array.isArray(input.imd))
			returnObj =  input.imd.length && input.imd.map(obj2=> obj2.c).join(', ');
		else
			returnObj = typeof returnFalseString !== 'undefined' ? returnFalseString : 'NA';
...
// arrayofintermitpoint for csv

materialAdmin.filter('arrayOfIntermitPointforcsv', function () {
	return function (input) {
		let returnObj;
		if (Array.isArray(input.imd))
			returnObj =  input.imd.length && input.imd.map(obj2=> obj2.c).join('/');
...
materialAdmin.filter('isShellTrackingDoc', [
	"$rootScope",
	function (
		$rootScope
	) {
	return function (customer) {
		...
		if(!($rootScope.$configs && $rootScope.$configs.tracking && $rootScope.$configs.tracking.custs || []).find(o => o._id == customer._id))
			return false;

		return !!customer.shipperEnterprise;
	}
}]);

materialAdmin.filter('getShipmentNum', [
	function (
	) {
	return function (invoices) {
		return Array.isArray(invoices) ? invoices.map(o => o.ref1).join(', ') : '';
	}
}]);
...
materialAdmin.filter('roundOff', function () {
	return function (input, str = 0, digit = 2) {
		if (typeof input !== 'number')
			return str;

		return (Math.round(input * Math.pow(10, digit)) / Math.pow(10, digit)) || 0;
	}
});
	...
	return function(aFormula=[], scope, formName){
		...
		aFormula.forEach( s => {
			...
			if(s === '+' || s === '*' || s === '/' || s ==='-' || s === '(' || s === ')')
				...
				str += (scope.invoice['rateChart' + key[0].toUpperCase() + key.slice(1)] || scope.invoice[key]);
			}else
...
// object key sum
materialAdmin.filter('sumOfCostAmt', function(){
	return function(obj = {}, key){
		...
		for(let k in obj){
			if(obj.hasOwnProperty(k))
				for(let j in obj[k][key])
				  sum+=(obj[k][key][j].amount || 0);
		}
