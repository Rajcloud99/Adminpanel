    ...
    function($rootScope, $stateParams, $state, $scope, $window, $location,
        ...
        $scope.$watch(function() {
            ...
            function(a){
                ...
                if($scope.currentPath == '/masters/icds'){
                    ...
                    if ($rootScope.objICD) {
                        ...
                        $state.go('masters.icd')
                    }
                } else if ($scope.currentPath == '/masters/icds/view'){
                    if ($rootScope.objICD) {
                        $scope.icd_show_add_button = true;
                        $rootScope.icdName = $rootScope.objICD.port_name ;
                        $scope.icd_show_no_data = false;
        ...
        $scope.selectICD = function (icd,index) {
            ...
            if (icd.port_name){
                ...
                $state.transitionTo('masters.icd.view',$stateParams,{reload:'masters.icd.view'});
            }
        };

        $scope.addICD = function () {
            $state.go('masters.icd.add');
        };
        ...
        $scope.onICDTypeAheadSelect = function($item, $model, $label){
            ...
            /*var sUrl = "#!/masters/icds/view/" + $scope.currentPage + "/" + $label;
            $rootScope.redirect(sUrl);*/
            $scope.getAllICDs(true);
        };
...
materialAdmin.controller("icdViewController",
    function($rootScope, $scope, $interval, $state , $stateParams, icdService, constants, growlService) {
        ...
        $scope.editICD = function(){
            ...
            if ($rootScope.objICD.name) {
                $stateParams.name = $rootScope.objICD.name ;
                $state.go('masters.icd.edit',{});
            }
        };
        ...
        $scope.deleteICD = function(){
            swal({
                ...
                text: "icd "+$scope.objICD.name+" will be removed from icd masters",
                type: "warning",
                ...
                function succDeleteICD(response){
                    if (response.message){
                        ...
                        $state.go('masters.icd',{},{reload:true});
                    }
                }
