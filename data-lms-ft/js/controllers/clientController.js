...
materialAdmin.controller(
	...
	function (
		...
		$scope.clients = [];
		$rootScope.misClientObj = "";
		$rootScope.misUserData = "";
		$rootScope.misUser = [];
		$rootScope.misClientEdit = false;
			...
			function (a) {
				...
				if ($scope.currentPath == "/clients/clientInfo") {
					...
					if ($rootScope.objClient) {
						...
						if (
							$rootScope.objClient &&
							$rootScope.objClient.client_display_name
						) {
							$rootScope.clientName = $rootScope.objClient.client_display_name;
						}
						$scope.client_show_no_data = false;
		...
		$rootScope.selectTabClient = function (index) {
			if (index === 6 && $scope.client_action_tag != "Edit") {
				$rootScope.misClientEdit = false;
				function success(response) {
					console.log(response);
					$rootScope.misConfigData = response;
				}

				function fail(response) {
					console.log(response);
				}
				let filter = { clientId: $rootScope.objClient.clientId };
				clientService.getMisConfig(filter, success, fail);
			}
			$rootScope.tab_active_client = index;
		};

		$rootScope.onSelect = function (formData) {
			if (formData) {
				if (!formData.contact_no || !formData.email) {
					swal(
						"Failed",
						"Selected user must have email Id and contact",
						"error"
					);
					$rootScope.misUserData = "";
					$rootScope.misUser = [];
				...
				let user = {
					...
					phoneNo: formData.contact_no,
					emailId: formData.email,
					_id: formData._id,
				};
                let flag=false;
                if($rootScope.misUser.length){
                     isInList = newList => $rootScope.misUser.filter(c => newList._id === c._id).length > 0;
                     flag=isInList(user);
                }
                if($rootScope.misUser.length>=3){
                    swal(
						"Failed",
						"Max 3 user can be selected",
						"error"
					);
                    $rootScope.misUserData = "";
                    return;
		...
		$rootScope.getUserByName = function (formData) {
			...
			if (formData) {
				function succGetUsers(response) {
					console.log(response);
					$rootScope.aUserName = response.data;
				}
				...
				let filter = {
					// created_by_employee_code: $rootScope.objClient.clientId,
					clientIdv2: [$rootScope.objClient.clientId],
					full_name: formData,
				};
				userService.getUsers(filter, succGetUsers, failGetUsers);
			}
		};
		...
		$rootScope.saveMisConfig = function (formData) {
			if (formData) {
				if (!$rootScope.misUser.length) {
					$rootScope.misUserData = "";
				}
				function success(response) {
					...
					if (response.status == "OK") $state.reload();
				}
				function fail(response) {
				...
				formData.user = $rootScope.misUser;

				if (!$rootScope.misClientEdit) {
					formData.type=formData.type?formData.type: $scope.$constants.misTypes[0];
					formData.serviceType=formData.serviceType?formData.serviceType: $scope.$constants.misServiceTypes[0];
				}
				...
				let filter = {
					clientId: $rootScope.objClient.clientId,
					...formData,
				};
				if ($rootScope.misClientEdit) {
					clientService.editMisConfig(filter, success, fail);
				} else {
					clientService.saveMisConfig(filter, success, fail);
				}
			}
		};

		$rootScope.editMisConfig = function (obj) {
			if (obj) {
				obj.user.forEach((o) => {
					o.full_name = o.name;
				});
				$rootScope.misClientObj = obj;
				$rootScope.misUser = obj.user;
				$rootScope.misClientEdit = true;
                $rootScope.aUserName=[];
			}
		};

        $rootScope.deleteUser = function (obj) {
			$rootScope.misUser.splice(obj, 1);
		};

		$rootScope.addMisConfig = function (obj) {
			$rootScope.misClientObj = "";
			$rootScope.misClientObj = obj;
		...
		$rootScope.deleteMisConfig = function (obj) {
			swal(
				{
				...
				function (isConfirm) {
					if (isConfirm) {
						if (obj) {
							...
							clientService.deleteMisConfig(filter, success, fail);
						}
					}
				}
			);
		...
		$scope.selectClient = function (client, index) {
			...
			if (client.client_display_name) {
				...
				$state.transitionTo("clients.clientInfo.view", $stateParams, {
					reload: "clients.clientInfo.view",
				});
			}
		};
		...
		$scope.getAllClients = function (autoSelect, queryObj, isPagination) {
			...
			if (!autoSelect) {
				var oFilter = { client_display_name: $scope.clientFullName };
			} else {
		...
		$scope.onClientTypeAheadSelect = function ($item, $model, $label) {
            ...
            $rootScope.redirect(sUrl);*/
			$scope.getAllClients(false);
			//$scope.clients=$item;
			//$scope.selectClient($item, 0);
		};
...
materialAdmin.controller(
	...
	function (
		...
		$scope.addVehTypeSel = function () {
				...
				for (var v = 0; v < $scope.aVehType.length; v++) {
					if ($scope.aVehType[v].avlClient == true) {
						...
						if (
							...
							$scope.aVehType[v].clientInfo.length > 0
						) {
						} else {
							$scope.aVehType[v].clientInfo = [];
						}
						...
						if (
							...
							for (var c = 0; c < $scope.aVehType[v].clientInfo.length; c++) {
								if (
									$scope.aVehType[v].clientInfo[c].clientId ==
									$rootScope.selectedClient.clientId
								) {
			...
			clientService.updateMultiVehTypes(
				{ vehTypes: $scope.aVehType },
				succ,
				fail
			);
		};
...
materialAdmin.controller(
	...
	function (
		...
		$scope.saveClient = function (form) {
			...
			if ($scope.tab_active_client !== 5) {
				//if document upload tab is not activ
				//  if(form.$valid){
				clientService.updateClient(
					$scope.objClient,
					successUpdateClient,
				...
				clientService.uploadClientDocs(
					data,
					successUpdateClient,
					failureUpdateClient
				);
...
materialAdmin.controller(
	...
	function (
		...
		$scope.editClient = function () {
			...
			$rootScope.misUserData = "";
			let obj = {
				type: $scope.$constants.misTypes[0],
				reportName: $scope.$constants.misReportName[0],
				serviceType: $scope.$constants.misServiceTypes[0],
				reportAs: $scope.$constants.misReportAs[0],
			};
			$rootScope.addMisConfig(obj);
			$rootScope.clearSearch();
			$scope.show_password_button = false;
			if (!$stateParams.skip) {
				$stateParams.skip = 1;
			...
			swal(
				{
					...
					text:
						"client " +
						$scope.objClient.name +
						" will be removed from client masters",
					type: "warning",
					showCancelButton: true,
