materialAdmin.controller('moneyReceiptTripMemoController', moneyReceiptTripMemoController);

moneyReceiptTripMemoController.$inject = [
	'$modal',
	'$parse',
	'$scope',
	'$stateParams',
...
function moneyReceiptTripMemoController(
	$modal,
	$parse,
	$scope,
	$stateParams,
	...
	(function init() {
		...
		if($stateParams.data){
			...
			getTrip($stateParams.data.gr.trip_no);

		}else{
			stateDataRetain.back('booking_manage.tripMemo');
			return;
		}
	...
	function getTrip(tripNo) {
		function success(res) {
            ...
            vm.selectedGr = angular.copy(vm.oTrip);
	...
	function submit(formData) {
		...
		console.log(vm.selectedGr);

		let request = {
			...vm.selectedGr,
			customer: (vm.selectedGr.customer && vm.selectedGr.customer._id) ? vm.selectedGr.customer._id: undefined,
			consignor: (vm.selectedGr.consignor && vm.selectedGr.consignor._id) ? vm.selectedGr.consignor._id : undefined,
			consignee: (vm.selectedGr.consignee && vm.selectedGr.consignee._id) ? vm.selectedGr.consignee._id : undefined,
			billingParty: (vm.selectedGr.billingParty && vm.selectedGr.billingParty._id) ? vm.selectedGr.billingParty._id : undefined ,
		...
		function success(res) {
			...
			stateDataRetain.back('booking_manage.tripMemo', res.data.data);
		}
