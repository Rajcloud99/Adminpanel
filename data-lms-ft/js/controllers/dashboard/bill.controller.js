materialAdmin
	.controller('billAnalyticsCtrl', billAnalyticsCtrl);

billAnalyticsCtrl.$inject = [
	'$timeout',
	'DatePicker',
	'dashboardFactory',
	'dashboardService'
...
function billAnalyticsCtrl(
	...
	vm.filter = {};
	vm.filter.aggregateByValue = 'Billing Party Wise';
	...
	vm.options = {
		chart: {
			...
			x: function(d){
				...
				return d._id[0].length > 16 ? d._id[0].substr(0,16)+'...' : d._id[0];
			},
	...
	function getAnalytics() {
		...
		dashboardService.getBillAnalytics(request, successCallback, failureCallback);
	...
	function prepareFilter() {
		...
		switch(vm.filter.aggregateByValue){
			...
			case 'Billing Party Wise' : request.aggregateBy = 'billing_party'; break;
			case 'Vendor Wise' : request.aggregateBy = 'vendor'; break;
			case 'Ownership Type Wise' : request.aggregateBy = 'ownership_type'; break;
		}
	...
	function showFullscreen() {
		dashboardFactory.openFullscreen({
			url: 'views/dashboard/billAnalytics.html',
			controller: 'billAnalyticsCtrl'
		}, vm);
	}
